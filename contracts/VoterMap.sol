...
contract VoterMap {
  ...
  mapping(address => mapping(uint => Node)) dllMap;
  ...
  */
  function insert(address voter, uint _prev, uint _curr, uint _next) internal {
    ...
    require(_prev == NULL_NODE_ID || contains(voter, _prev));
    require(_next == NULL_NODE_ID || contains(voter, _next));

    require(getNext(voter, _prev) == _next);
    require(getPrev(voter, _next) == _prev);
